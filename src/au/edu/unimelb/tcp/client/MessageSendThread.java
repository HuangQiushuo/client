...
public class MessageSendThread implements Runnable {
	...
	public MessageSendThread(SSLSocket sslsocket, State state, boolean debug) throws IOException {
	...
	public void run() {
		...
		try {
			...
			MessageSend(sslsocket, "#login " + state.getUserId() + " " + state.getPassword());			
			Thread.sleep(2000);
			// send the #newidentity command
			MessageSend(sslsocket, "#newidentity " + state.getIdentity());
	...
	public void MessageSend(Socket socket, String msg) throws IOException {
		...
		// add this branch
			...
			if(array[0].startsWith("#login")) {
				sendToServer = ClientMessages.getLoginRequest(array[1], array[2]);
				send(sendToServer);
			}
			else {
				System.out.println("Invalid command!");
	...
	public void switchServer(SSLSocket temp_sslsocket, DataOutputStream temp_out) throws IOException {
